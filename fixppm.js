...
// @version      1.8.24
// @description  Fix 2024 - add type correction map for parsing xls 2.
...
(function () {
    ...
    //var publicHolidays24 = ["01-01", "01-06", "03-31", "04-01", "05-01", "05-03", "05-19", "05-30", "08-15", "11-01", "11-11", "12-25", "12-26"];
    ...
    var publicHolidays = ["[^1-9]1/1$", "[^1-9]1/6", "[^1-9]3/31", "4/1$", "5/1$", "5/3$", "5/19", "5/30", "8/15", "11/1$", "11/11", "12/25", "12/26"];

    //---------------------------------- FIX NAMES JIRA-PPM
    console.log(">>> There are some typos in ppm or jira, so here is mapping - how jira's name shall be mapped to ppm's name. It will be used in xls2 parsing. These are full names, check in exporte xml 2.")
    const ppmFullNameVSexpectedJiraFullName = new Map();
    ppmFullNameVSexpectedJiraFullName.set("ppmFullName 1", "jiraFullName 1")
    ppmFullNameVSexpectedJiraFullName.set("ppmFullName 2", "jiraFullName 2")

    // Save data
    let jiraNamePPMTaskNameDIV = document.createElement('div');
    ppmFullNameVSexpectedJiraFullName.forEach((ppmName, jiraName) => {
        let entryDiv = document.createElement('div');
        entryDiv.textContent = `Key: ${jiraName}, Value: ${ppmName}`;
        entryDiv.style.fontSize = '5px';
        entryDiv.style.color = 'rgb(127,127,127)';
    ...
    //let containerDiv = document.getElementById('jiraNamePPMTaskName');
    //let newMap = new Map();
    //Array.from(containerDiv.children).forEach(childDiv => {
    //    let content = childDiv.textContent;
    //    let [keyPart, valuePart] = content.split(', ');
    //    let key = keyPart.replace('Key: ', '').trim();
    //    let value = valuePart.replace('Value: ', '').trim();
